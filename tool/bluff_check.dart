...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final guestPlayer = ((await guest.rpc('join_room', params: {
      'p_code': code, 'p_name': 'Giulia', 'p_color': 'coral',
    }) as List).first as Map<String, dynamic>)['player_id'] as String;
    final thirdPlayer = ((await third.rpc('join_room', params: {
      'p_code': code, 'p_name': 'Marco', 'p_color': 'aqua',
    }) as List).first as Map<String, dynamic>)['player_id'] as String;

    await host.rpc('start_game', params: {'p_room': roomId, 'p_game': 'bluff_story'});

    final round = await host.from('rounds').insert({
      'room_id': roomId,
    ...
    final seenByGuest = await guest.from('votes').select('value').eq('round_id', roundId);
    check('la verità del narratore arriva agli altri', seenByGuest.isNotEmpty,
        '${seenByGuest.length} voti visibili');

    // uno indovina (indice 0), l'altro sbaglia (indice 1)
    await guest.from('votes').insert({
      'round_id': roundId, 'player_id': guestPlayer, 'value': {'pick': 0},
    });
    await third.from('votes').insert({
      'round_id': roundId, 'player_id': thirdPlayer, 'value': {'pick': 1},
    });

    // il client non può assegnarsi punti da solo
    try {
      await guest.from('players').update({'score': 99}).eq('id', guestPlayer);
      final after = (await guest.from('players').select('score').eq('id', guestPlayer)).first;
      check('un giocatore non può assegnarsi punti',
          (after['score'] as num).toInt() == 0, '${after['score']}');
    } on PostgrestException catch (_) {
      check('un giocatore non può assegnarsi punti', true);
    }

    // i punti li assegna la RPC, solo prima del reveal
    await host.rpc('award_points', params: {
      'p_round': roundId,
    ...
    check('chi indovina prende 2 punti', scores[guestPlayer] == 2, '${scores[guestPlayer]}');
    check('il narratore prende 1 punto per ogni bluff riuscito',
        scores[hostPlayer] == 1, '${scores[hostPlayer]}');
    check('chi sbaglia non prende punti', scores[thirdPlayer] == 0, '${scores[thirdPlayer]}');
    ...
    check('i punti non si assegnano due volte', scores[guestPlayer] == 2,
        '${scores[guestPlayer]}');

    // un non-host non può assegnare punti
    try {
      await guest.rpc('award_points', params: {
        'p_round': roundId,
        'p_awards': {guestPlayer: 50},
      });
      final after = (await guest.from('players').select('score').eq('id', guestPlayer)).first;
      check('un non-host non può assegnare punti',
          (after['score'] as num).toInt() == 2, '${after['score']}');
    } on PostgrestException catch (_) {
    ...
    await host.rpc('start_game', params: {'p_room': roomId, 'p_game': 'bluff_story'});
    final reset = await host.from('players').select('score').eq('room_id', roomId);
    check('una nuova partita riparte da zero punti',
