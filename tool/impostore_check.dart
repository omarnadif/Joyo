...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    await host.rpc(
      'start_game',
      params: {'p_room': roomId, 'p_game': 'impostore'},
    );
    await host.rpc(
    ...
    final round =
        (await host.from('rounds').select('id,content').eq('room_id', roomId))
            .first;
    final roundId = round['id'] as String;
    final content = round['content'] as Map<String, dynamic>;
    check(
      'la parola non finisce nel contenuto pubblico del round',
      !content.containsKey('word'),
      content.keys.join(', '),
    );

    // ognuno vede solo la propria riga di segreto
    final hostSecrets = await host
        .from('round_secrets')
        .select('player_id,payload')
        .eq('round_id', roundId);
    final guestSecrets = await guest
        .from('round_secrets')
        .select('player_id,payload')
        .eq('round_id', roundId);
    check(
      'ogni giocatore vede una sola riga di segreto',
      hostSecrets.length == 1 && guestSecrets.length == 1,
      'host=${hostSecrets.length} guest=${guestSecrets.length}',
    );
    check(
      'ognuno vede la riga giusta',
    ...
    check(
      'l\'impostore non riceve la parola',
      payloads.every(
        (p) => p['impostor'] == true
            ? !p.containsKey('word')
            : p['word'] == 'Spiaggia',
    ...
    check(
      'non si possono leggere i segreti degli altri',
      spy.isEmpty,
      '${spy.length} righe',
    );
    ...
    try {
      ...
      check(
        'i segreti non sono scrivibili dal client',
        false,
        'insert riuscito',
      );
    ...
    check(
      'chi ha smascherato prende 2 punti',
      accusers.every((e) => e.value == 2),
      accusers.map((e) => e.value).join('/'),
    );
    check(
      'l\'impostore scoperto ma che indovina recupera 3 punti',
      scores[impostorPlayer] == 3,
      '${scores[impostorPlayer]}',
    );
    ...
    final round2 =
        (await host.from('rounds').select('id').eq('room_id', roomId))
                .first['id']
            as String;
    ...
    final impostor2 = secrets2.entries
        .firstWhere((e) => e.value['impostor'] == true)
        .key;
    ...
    check(
      'l\'impostore non scoperto prende 5 punti',
      scores2[impostor2] == 5,
      '${scores2[impostor2]}',
    );
